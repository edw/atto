...
void lexer_next_char(lexer_state *s);

void init_lexer(lexer_state *s, get_char_fn get_char, unget_char_fn unget_char) {
  s->get_char = get_char;
...
void lexer_append(lexer_state *s, int new_ch) {
  char ch = (char)new_ch;
...
int lexer_peek(lexer_state *s) {
  int ch = s->get_char(s->cookie);
  s->unget_char(s->cookie, ch);
  return ch;
}

void lexer_abort(lexer_state *s, char *message) {

}

void token_set(token* t, lexer_state *s, int token_type) {
  t->chs = s->token_chs;
...
void lexer_skip_whitespace_and_comments(lexer_state *s) {
  do {
...
void lexer_reset_token(lexer_state *s) {
  if (s->token_chs_cap == -1) {
    s->token_chs = calloc(1, sizeof(char));
    if (s->token_chs == NULL)
      exit(EXIT_FAILURE);
...
int lexer_get_token(lexer_state *s, token *t) {
...
int main(int argc, char **argv) {
  ...
  init_lexer(&lex, stdin_get_char, stdin_unget_char);

  while (lexer_get_token(&lex, &token)) {
    description = token_description_alloc(&token);
    if (description == NULL)
      exit(EXIT_FAILURE);
    printf("%s\n", description);
    free(description);
